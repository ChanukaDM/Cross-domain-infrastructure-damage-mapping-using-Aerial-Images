...
from torchgeo_common_linz import (
    ...
    lora_target_modules, model_family, compute_metrics, roc_auc, threshold_sweep,
    fbeta, roc_curve, pr_curve, best_threshold_youden, average_precision,
)
import sys
...
# Grad-CAM overlays: pick the last spatial layer per backbone family.
def _gradcam_target(m, name):
    core = getattr(getattr(m, "base_model", m), "model", m)   # unwrap PEFT if present
    fam = model_family(name)
    try:
        if fam == "resnet": return core.layer4, "bchw"        # torchvision ResNet
        if fam == "swin":   return core.features, "bhwc"      # torchvision Swin-V2 (channels-last)
        if fam == "vit":    return core.blocks[-1], "tokens"  # timm ViT (token grid)
    except Exception:
        pass
    return None, None

_tgt, _layout = _gradcam_target(model, MODEL_NAME)
run_gradcam(model, {c[2]: c[0] for c in crops}, GRADCAM_IMAGES,
            target_layer=_tgt, layout=_layout,
            model_tag=MODEL_NAME, out_dir=GRADCAM_DIR, device=DEVICE)
